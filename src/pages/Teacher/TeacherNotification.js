...
const TeacherNotification = () => {
  ...
  const handleSubmitForm = async () => {
    if (!description || !topic) {
      ...
      try {
        const res = await axios.post(`${BASE_URL}/create-notification`, {
          idTeacher: _id,
          title: topic,
  ...
  const handleUpdateForm = async () => {
    if (!description || !topic) {
      ...
      try {
        const res = await axios.put(`${BASE_URL}/edit-notification`, {
          idTeacher: _id,
          title: topic,
          description: description,
  ...
  const handleActiveStatus = async (status, id) => {
    try {
      const res = await axios.put(`${BASE_URL}/update-status-notification`, {
        isShow: !status,
        idNotification: id,
      });
      if (res.status === 200) {
